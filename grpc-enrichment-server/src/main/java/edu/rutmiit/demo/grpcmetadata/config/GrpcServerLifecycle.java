package edu.rutmiit.demo.grpcmetadata.config;

import edu.rutmiit.demo.grpcmetadata.service.CarMetadataServiceImpl;
 ...
 *
 * gRPC-сервер запускается отдельно от Spring MVC/Tomcat
 * и слушает собственный TCP-порт (по умолчанию 9090).
 *
 * Сервис используется для enrichment metadata автомобилей:
 * demo-rest -> RabbitMQ -> enrichment-consumer -> gRPC ResolveSpecifications()
 */
@Component
    ...
    public void start() {
        ...
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService(new CarMetadataServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC metadata server started on port {}", grpcPort);
            log.info("Service: CarMetadataService.ResolveSpecifications()");
