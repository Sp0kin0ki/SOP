...
public class EventNotificationListener {
    ...
    private String buildDescription(String eventType, JsonNode payload) {
        try {
            return switch (eventType) {
                ...
                case "car.created" -> {
                    CarEvent.Created e =
                            jsonMapper.treeToValue(payload, CarEvent.Created.class);

                    yield "Создан автомобиль %s %s (%d)"
                            .formatted(e.brand(), e.model(), e.year());
                }

                case "car.updated" -> {
                    CarEvent.Updated e =
                            jsonMapper.treeToValue(payload, CarEvent.Updated.class);

                    yield "Обновлён автомобиль id=%d"
                            .formatted(e.carId());
                }

                case "car.deleted" -> {
                    CarEvent.Deleted e =
                            jsonMapper.treeToValue(payload, CarEvent.Deleted.class);

                    yield "Удалён автомобиль id=%d"
                            .formatted(e.carId());
                }

                case "car.enriched" -> {
                    CarEvent.Enriched e =
                            jsonMapper.treeToValue(payload, CarEvent.Enriched.class);

                    yield "Автомобиль id=%d дополнен характеристиками (%d л.с., %s)"
                            .formatted(
                                    e.carId(),
                                    e.horsepower(),
                                    e.engine()
                ...
                case "dealer.created" -> {
                    DealerEvent.Created e =
                            jsonMapper.treeToValue(payload, DealerEvent.Created.class);

                    yield "Создан дилер в городе %s"
                            .formatted(e.city());
                }

                case "dealer.deleted" -> {
                    DealerEvent.Deleted e =
                            jsonMapper.treeToValue(payload, DealerEvent.Deleted.class);

                    yield "Удалён дилер из города %s (удалено автомобилей: %d)"
                            .formatted(
                                    e.city(),
                                    e.deletedCarsCount()
                            );
    ...
    private String resolveIcon(String eventType) {
        return switch (eventType) {
            case "car.created"   -> "car-plus";
            case "car.updated"   -> "car-edit";
            case "car.deleted"   -> "car-remove";
            case "car.enriched"  -> "analytics";
            case "dealer.created" -> "dealer-plus";
            case "dealer.deleted" -> "dealer-remove";
            default -> "bell";
        };
    }
    ...
    private String resolveLevel(String eventType) {
        return switch (eventType) {
            case "car.deleted", "dealer.deleted" -> "warning";
            case "car.enriched" -> "info";
            default -> "success";
        };
    }
